...
const resolveCurrencyPreference = (req) => {
  ...
  const locale = String(
    req.user?.locale || req.headers["x-user-locale"] || "",
  ).toLowerCase();
...
const handleControllerError = (res, error) => {
  if (res.headersSent) {
    console.warn(
      "[AgentController] Headers already sent, suppressing extra error response.",
    );
    return;
  }
  ...
  console.error(
    ...
    sanitizeLog(error?.message || error),
  );

  const status =
    error?.status ||
    error?.response?.status ||
    (error?.isConfigError ? 400 : 500);
  const is429 =
    status === 429 ||
...
export const generateResponse = async (req, res) => {
  ...
  console.log("[PDF Agent Service]", {
    ...
    fileName:
      (req.file || artifactFile)?.originalname ||
      (req.file || artifactFile)?.name,
  ...
  if (inFlightRequests.has(dedupeKey)) {
    console.log(
      "[agent.controller] Deduplicated in-flight request:",
      dedupeKey,
    );
    try {
  ...
  const requestPromise = (async () => {
    ...
    const responseText =
      result?.aiResponse || "No response received from selected agent.";
    ...
    const finalResponseType =
      result?.responseType || (finalImages.length > 0 ? "image" : "text");
    const finalImageUrl =
      result?.imageUrl ||
      finalImages[0]?.url ||
